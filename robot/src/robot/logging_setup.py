"""Console and file log formatting for the robot process.

The console stays human-readable for an operator watching ``just run-robot``.
...
Rotated files keep the ``.log`` extension: ``robot.2026-07-27.log`` instead of
the stdlib default ``robot.log.2026-07-27``. Python's rotation cleanup
regenerates candidate names through this same namer, so daily retention keeps
working.

Deliberately duplicated from the server package: the robot is a standalone
client and shares no code with the server, only the HTTP contract.
"""
...
def rotated_log_name(default_name: str) -> str:
    ...
    Args:
        default_name: Name the stdlib would use, e.g. ``logs/robot.log.2026-07-27``.

    Returns:
        The same path as ``logs/robot.2026-07-27.log``.
    """
...
def build_file_handler(path: Path, retention_days: int) -> logging.Handler:
    ...
    Args:
        path: Active log file, e.g. ``logs/robot.log``.
