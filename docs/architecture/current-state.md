...
| Speaker recognition | Absent (runtime); PC-3A study closed (provisional PASS) on an unmerged branch | STT and VAD exist; on `main` no speaker enrollment, voiceprint, verification model, or runtime identity adapter exists, and `IdentityEvidenceSource.VOICE` stays untrusted. Plan 0047 (PC-3A) ran on `feat/0047-speaker-calibration` (PR pending, nothing merged) as an out-of-runtime study under `scripts/speaker_calibration*` (`just speaker-calibration`): a private corpus CLI, a frozen offline backend (SpeechBrain `1.1.1` `EncoderClassifier`, ECAPA revision `0f99f2d0…`, CPU) and an aggregate analysis. Measured on 62 private samples (reference 6, genuine 24, live impostor 24 from four consenting adults, replay 8; deleted afterwards): threshold 0.4834, 0/24 impostor false accepts, 0/24 genuine false rejects (in-sample), 6/8 replay probes accepted, frozen-protocol p95 231 ms vs the 500 ms budget — a **provisional PASS**. Replay acceptance means voice is not standalone high-assurance evidence. No enrollment, voiceprint storage or runtime wiring exists; PC-3B and PC-4 stay open. |
