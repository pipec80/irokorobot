...
> **Observed:** implementation and plan status aligned 2026-09-08 (Plan 0046
> CM-0 benchmark and its measured RED baseline; no runtime memory change).
>
...
| Personal owner/children/PIN setup (Plan 0025) | Implemented/local-only | `just setup-personal` bootstraps Pipec as sole owner, confirms the owner's children as active v4 `child_of` relations, and stores one scrypt-hashed PIN credential (`owner_pin_credentials`, migration 006). Idempotent rerun and PIN rotation are covered. |
...
| Speaker recognition | Absent (runtime); PC-3A study in progress on an unmerged branch | STT and VAD exist; on `main` no speaker enrollment, voiceprint, verification model, or runtime identity adapter exists, and `IdentityEvidenceSource.VOICE` stays untrusted. Plan 0047 (PC-3A) runs on `feat/0047-speaker-calibration` (no PR, nothing merged): Tasks 0–3 are complete out of runtime under `scripts/speaker_calibration*` — the numeric layer, the private-corpus CLI, and (Task 3, 2026-09-08) the frozen offline backend (`speaker_calibration_backend.py`: SpeechBrain `1.1.1` `EncoderClassifier`, ECAPA revision `0f99f2d0…`, CPU, cached-then-offline). Task 3 is a **technical PASS** — `model-contract` resolves the pinned revision and embeds a non-biometric tone to a finite 192-d vector; the frozen latency protocol measured p50 222.6 ms / **p95 231.2 ms** vs the precommitted p95 ≤ 500 ms budget. Task 4 (2026-09-14) captured a private, untracked owner corpus (reference 6/6, genuine 24/24) and Pipec's replay probes (8/6); the capture CLI now rejects utterances under 2.5 s and warms the VAD before the countdown. No household or impostor voices yet, no FAR/FRR, no runtime wiring. The PC-3A measurement outcome (or honest FAIL) is still unwritten and waits on the 18 live-impostor samples (Task 5, three consenting adults pending) or Task 7 closure; this row moves out of "in progress" when that lands. |
| Longitudinal-memory benchmark (CM-0) | Benchmark implemented; runtime unchanged; measured RED | Plan 0046 (closed 2026-09-08) repaired the two stale evaluator entrypoints, added `scripts/eval_longitudinal_memory.py` (an out-of-runtime observation instrument: strict suite loader + privacy validator + current-capability driver + deterministic scorer + safe runner + report) and the synthetic 9-scenario suite `tests/evals/golden_longitudinal_memory.yaml`, and recorded the first reproducible RED baseline at `ac43c58` (`just eval-longitudinal --runs 3`, exit `1`): single-turn extraction is the only live seam and scores precision/recall `0.25`, every other longitudinal operation is honestly `unsupported`, all four frozen gates `FAIL`. The production database was not touched. See [`docs/evals/0046-longitudinal-memory-baseline.md`](../evals/0046-longitudinal-memory-baseline.md). No memory capability, authorization, or runtime behaviour changed. |
