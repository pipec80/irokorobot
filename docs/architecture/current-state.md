...
| Server HTTP/ASGI hardening (Plan 0031 capsule) | Closed 2026-09-03 | `create_app()` + a failure-safe `lifespan()` own every shared resource (httpx client, STT/TTS models, DB); `GET /ready` is a side-effect-free readiness probe distinct from `/health`; every route's real error codes are documented via OpenAPI; uploads are bounded twice (raw body + per-file); every SQLite write goes through one owned transaction primitive. Full narrative, measured evidence, and ADR cross-references: [`server-production-baseline.md`](server-production-baseline.md). |
