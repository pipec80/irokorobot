...
| 3 | Biometrics | PC-3B — consented speaker runtime evidence | Local enrollment, consent, revocation and verification produce typed `VOICE` evidence; missing, weak or failed evidence remains `unknown` and grants no capability directly. | PC-3A measured and accepted | Required | **Draft 2026-09-25** — re-audited against merged code under queue rule 4/6 and written as [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md). The product outcome is unchanged; the re-audit added cost and shape the row did not price (`VOICE` is already unresolvable, so `resolve_active_person` is untouched; the frozen backend is a dev dependency, not a server one; the identity resolver is composed before the audio is read; the study's model constant may not be copied into runtime code). Eight decisions (D-1…D-8) are Pipec's; the plan is not executable until it is promoted | [Plan 0015](../plans/open/0015-personal-companion-design.md#pc-3--speaker-evidence--pc-3a-closed-pc-3b-unplanned); [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md) |
...
| Step | Work | Why here |
|---:|---|---|
| 0 | Accept ADR-0015; re-audit PC-3B and write its plan | Decisions and documents, no code. **Done 2026-09-25** — ADR-0015 accepted; PC-3B re-audited and drafted as [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md) |
...
revocation and runtime evidence); its rule-6 re-audit ran on 2026-09-25 against
merged code and produced [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md),
a `Draft` that waits for Pipec's decisions D-1…D-8 and for promotion. Rows 4–19
are not executable and their plan numbers are intentionally unreserved.
