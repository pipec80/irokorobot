...
| 2 | Biometrics | PC-3A — speaker calibration study | A frozen local backend and versioned genuine/impostor corpus measure FAR/FRR, replay, quality, failure posture and CPU feasibility without granting identity or authorization. | CM-0 closed; Plan 0047 readiness amendment approved | Required | **Complete 2026-09-25** — Plan 0047 **PROVISIONAL PASS** (2026-09-25): frozen SpeechBrain `1.1.1` ECAPA (`0f99f2d0…`) on CPU; 62 private samples (reference 6, genuine 24 over two sessions and four conditions, live impostor 24 from four consenting adults, replay 8) give a selected cosine-distance threshold of 0.4834 with 0/24 live-impostor false accepts, 0/24 genuine false rejects (in-sample, zero by construction; separation between the farthest genuine and the closest impostor distance 0.342) and 6/8 replay probes accepted; the frozen latency protocol measured p95 231 ms < 500 ms. Voice is not standalone high-assurance evidence and `VOICE` stays untrusted; PC-3B and PC-4 are still open (executed on `feat/0047-speaker-calibration`, merged as PR #134 / `de4c6cf`) | [Plan 0015](../plans/open/0015-personal-companion-design.md#pc-3--speaker-evidence--pc-3a-closed-pc-3b-unplanned); [Plan 0047](../plans/completed/0047-speaker-evidence-calibration-study.md) |
| 3 | Biometrics | PC-3B — consented speaker runtime evidence | Local enrollment, consent, revocation and verification produce typed `VOICE` evidence; missing, weak or failed evidence remains `unknown` and grants no capability directly. | PC-3A measured and accepted | Required | **`NOW` since 2026-09-25** — re-audited against merged code under queue rule 4/6, written as [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md) and promoted by Pipec with every decision answered. The product outcome is unchanged; the re-audit added cost and shape the row did not price (`VOICE` is already unresolvable, so `resolve_active_person` is untouched; the frozen backend is a dev dependency, not a server one; the identity resolver is composed before the audio is read; the study's model constant may not be copied into runtime code). Eight tasks, each RED first, behind `SPEAKER_AUTHENTICATION_ENABLED=false`; `VOICE` stays untrusted and `cognition/identity.py` is not edited | [Plan 0015](../plans/open/0015-personal-companion-design.md#pc-3--speaker-evidence--pc-3a-closed-pc-3b-unplanned); [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md) |
...
|---|---|---|---|
| A. The memory path of the portfolio (amended 2026-09-25) | PC-3B, PC-4 and CM-1…CM-7 are closed (the other rows of the 19 continue after the purchase; see below) | The portfolio table | 2 of 19 closed (CM-0, PC-3A); [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md) (PC-3B) is `NOW` since 2026-09-25 |
...
| Step | Work | Why here |
|---:|---|---|
| 0 | Accept ADR-0015; re-audit PC-3B and write its plan | Decisions and documents, no code. **Done 2026-09-25** — ADR-0015 accepted; PC-3B re-audited, written as [Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md), its eight decisions answered and the plan promoted to `NOW` |
...
evidence), had its rule-6 re-audit on 2026-09-25 against merged code; it became
[Plan 0053](../plans/open/0053-consented-speaker-runtime-evidence.md), Pipec
answered its eight decisions and promoted it, so **Plan 0053 is `NOW`**. Rows
