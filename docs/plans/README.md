...
| Lane | Plans | Meaning |
|---|---|---|
| **NOW** | [0034 — upload and multipart security](open/0034-upload-and-multipart-security.md) | Promoted 2026-09-02 with Pipec's explicit authorization. Implementation and gates complete — 987 tests, lint, typecheck, audit green. A raw ASGI body-limit middleware plus per-file bounded reads closed an unbounded-memory read on 4 upload sites and a real gap the RED test caught: with face auth off by default, an oversized `frame` field was never read or sized by any per-file check. Real acceptance still pending. |
...
| **QUEUED** | [0035 — SQLite transaction owner](open/0035-sqlite-transaction-owner.md) | Next child in the order locked by 0031: one safe runtime transaction primitive. Promotion needs Plan 0034 closed plus Pipec's explicit authorization. Children 0036–0042 remain transitively queued. |
...
| 15 | [0034 — upload and multipart security](open/0034-upload-and-multipart-security.md) | Promoted 2026-09-02, implementation and gates complete | Real acceptance pending; not yet closed |
