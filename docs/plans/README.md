...
| Lane | Plans | Meaning |
|---|---|---|
| **NOW** | [0032 — server privacy and request observability](open/0032-server-privacy-and-request-observability.md) | Promoted 2026-09-02 with Pipec's explicit authorization. First child of the server-production capsule and the first plan in the chain that touches production code. Implementation and gates complete — 953 tests, lint, typecheck, audit and all 17 hooks green; real voice-turn acceptance still pending. |
...
| **QUEUED** | [0033 — owner unlock HTTP hardening](open/0033-owner-unlock-http-hardening.md) | Next child in the order locked by 0031: deterministic PIN validation and a race-safe limiter. Promotion requires Plan 0032 closed plus Pipec's explicit authorization. Children 0034–0042 remain transitively queued. |
