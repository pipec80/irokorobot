...
- confirmed entities and directional `child -> child_of -> owner`
  relationships;
- onboarding completion and enabled authentication methods;
...
1. [Plan 0025](0025-personal-owner-bootstrap-and-pin-setup.md) — persistent
   personal profile, Pipec as owner, confirmed child relations, and local PIN
   setup;
...
Connect the existing owner bootstrap, onboarding state, and local recovery
entry point. Establish Pipec before accepting any household relationship.
Confirm and store Máximo and Dominga as entity relationships, not as a prompt
sentence or vector-only memory.
