...
**Status:** Ready for owner review. Do not implement until Pipec explicitly
authorizes execution.

**Goal:** Provide one restart-safe, local CLI wizard that establishes Pipec as
the sole owner, stores confirmed personal/child relationships, completes the
existing onboarding frontier, and persists a securely hashed PIN credential.

**Architecture:** A local application service writes existing v4 entities,
facts, relations, owner role, and operational flags. A dedicated credential
repository stores only a salted scrypt verifier in a new additive migration.
The CLI is a thin `getpass`/input adapter; it never becomes an HTTP or general
family administration API.
...
- `meta.onboarding_complete` is set only after the complete personal checklist
  has been supplied or explicitly declined where optional. Do not introduce a
  weaker `personal_mvp_ready` shortcut.
- Re-running the same confirmed setup must not create duplicate entities,
  relations, role assignments, or credentials.
...
- repeating save for the same owner updates/rotates atomically instead of
...
Validate owner status using the existing role table before writing. Rotation
must use `BEGIN IMMEDIATE`, revoke the previous active row, insert the new row,
and commit once. On any exception, roll back and re-raise.
...
### Task 4: Build the confirmed personal onboarding application service
...
- Produces one owner, v4 facts/relations, owner role, PIN credential, and
  `meta.owner_name`/`meta.onboarding_complete`.
...
- birth date, home, workplace, and likes use closed v4 predicates;
- optional declines do not invent partner/pet entities;
- active PIN credential exists but plaintext does not appear anywhere in the
  DB file's logical rows;
- `owner_name == "Pipec"` and `onboarding_complete == "true"` are set last.
...
async def test_setup_rerun_is_idempotent(...): ...
async def test_setup_does_not_mark_complete_when_a_required_value_is_invalid(...): ...
```

The rerun must keep one owner, one credential, and two relations.
...
1. validate the complete immutable input without writing;
...
4. write `birth_date` and `likes` through `assert_literal_fact()`;
5. create place/organization/person/pet entities and their closed v4
   relations;
6. write each child as `source child -> child_of -> target owner`;
7. hash and persist the PIN;
8. set `owner_name`;
9. set `onboarding_complete="true"` only after rereading and verifying every
   required record.

Use `resolve_predicate()` and fail if a closed predicate is absent. Do not
write relationship sentences into prompts, semantic memory, or vectors.
...
- prompts follow owner → required profile → optional relations → children →
  PIN → redacted summary → literal `SI` confirmation;
- PIN mismatch writes nothing;
- any confirmation other than `SI` writes nothing;
- summary includes names/profile but displays PIN only as `******`;
...
- `status` prints only schema version, owner count, active child-relation count,
  onboarding-complete state, and active credential count;
...
`try/finally`, and use `getpass.getpass` for PIN entries. The write transaction
must acquire SQLite `BEGIN IMMEDIATE` with the configured bounded busy timeout;
lock acquisition failure exits non-zero before any setup write. Add a `status`
subcommand that performs only the five non-secret aggregate/status reads named
above.
...
- Modify: `docs/plans/0025-personal-owner-bootstrap-and-pin-setup.md`
...
- the complete onboarding flag is set only after all required inputs;
