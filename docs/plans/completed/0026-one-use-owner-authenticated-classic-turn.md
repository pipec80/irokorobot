...
- Treat Plan 0025 readiness as derived from exactly one active owner, the
  confirmed active child relations, and one active credential. Do not require
  or set legacy `meta.onboarding_complete` in this authentication slice.
...
- The grant has no memory-mutation, biometric, home-control, PC-administration,
  or actuator capability. No code may interpret authentication or the owner
  role as a master permission.
...
- the resulting scope contains only `PERSONAL_PROTECTED_READ` plus
  `child_data`, with no general action capability;
...
- Modify: `docs/plans/open/0026-one-use-owner-authenticated-classic-turn.md`
...
- the token can authorize only the named child-data read and cannot be reused
  as authority for mutation or unrelated digital/physical actions;
